			...
			if (currentUser && currentUser.id) {
				try {
					...
					if (result && (result as { success?: boolean }).success !== false) {
						...
						const userData = (result as { user?: unknown })?.user || result;
						if (userData && typeof userData === 'object' && 'id' in userData) {
							// Map backend response to frontend format
							const updatedUser = {
								id: (userData as { id: number }).id,
								email:
									(userData as { email?: string }).email || currentUser.email,
								name: (userData as { name?: string }).name || currentUser.name,
								specializationId:
									(userData as { specialization_id?: number })
										.specialization_id || parseInt(selectedSpecialization),
								readinessScore:
									(userData as { readiness_score?: number }).readiness_score ||
									currentUser.readinessScore ||
									0,
								technicalScore:
									(userData as { technical_score?: number }).technical_score ||
									currentUser.technicalScore ||
									0,
								softSkillsScore:
									(userData as { soft_skills_score?: number })
										.soft_skills_score ||
									currentUser.softSkillsScore ||
									0,
								leadershipScore:
									(userData as { leadership_score?: number })
										.leadership_score ||
									currentUser.leadershipScore ||
									0,
								createdAt:
